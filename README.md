...
This project is available as a web server. Click [here](https://cmembuilder.com) for more information.
...
```bash
python tools/refpoints.py example/1xq8_ppm.pdb
```
...
```bash
grep -E '^ATOM|^TER' example/1xq8_ppm.pdb > clean.pdb
...
```bash
python tools/fit_to_ref.py example/1xq8_ppm.pdb cg.pdb cg_aligned.pdb
```
