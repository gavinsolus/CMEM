...
import os
from tools.cm_parser import main as parser_main
...
def get_cut_pannel_lipids(cut_value, cut_pannel, lipid_list, lc_s4_start, lc_s8_start, surface_points, surface_points_normals, mdist, points_occupy, orientation=1, bin_area=0.6, lastp=False):
    ...
    if lastp:
        ...
        if cut_curve_length >= lipd_o:
            lipd = get_lipd(cut_curve_length, bin_area = bin_area)
        else:
            lipd = lipd_o
...
def generate_lipids(diagonal_points, lipid_list, surface_points, surface_points_normals, mdist, lipid_orientation, bin_area=0.6):
    ...
    for index, point in enumerate(diagonal_points):
        if index == 0:
            start_point = point
            cut_value   = point[0]
            s_points, s_lipids, lc_s4, lc_s8 = get_cut_pannel_lipids(cut_value, "XZ", lipid_list, lc_s4_start, lc_s8_start, surface_points, surface_points_normals, mdist, lipids, orientation=lipid_orientation, bin_area=bin_area)
            ...
            if dist > d_lipd_o - 2 and point[0] > boundary_value - 2:
                start_point = point
                cut_value = point[0]
                s_points, s_lipids, lc_s4, lc_s8 = get_cut_pannel_lipids(cut_value, "XZ", lipid_list, lc_s4_start, lc_s8_start, surface_points, surface_points_normals, lipids, mdist, orientation=lipid_orientation, bin_area=bin_area, lastp=True)
...
def generate_lipid2atom(length_lnd, lipid_dict, lipid_folder="lipid_lib"):
    ...
    for lipid in lipid_list:
        try:
            ...
            globals()[f"lipid_{lipid}"] = lipid_data  
...
if __name__ == "__main__":
    ...
    c_threshold          = 3.0
    bin_area = 0.6
    lipd_o = math.sqrt(bin_area) * 10
    boundary_value = 0.5*(box_X-math.sqrt(bin_area)*10 + 0.5)
